 ...
 */
function ensureAdminSetup(siteDir, org) {
  const adminDir = path.join(siteDir, 'public', 'admin');
...
<html>
  ...
  <body>
    ...
    console.log(`✓ Created: admin/index.html for ${org.name}`);
  }
  ...
  if (fs.existsSync(baseLayoutPath)) {
    ...
    if (!content.includes('netlify-identity-widget.js')) {
      ...
      console.log(`✓ Updated: BaseLayout.astro with Netlify Identity widget for ${org.name}`);
    }
  }

  console.log(`✓ Admin setup complete for ${org.name}`);
}
