...
function SelectTrigger({ children, className, ...props }) {
    return (<SelectPrimitive.Trigger data-slot="select-trigger" className={cn("flex w-full items-center justify-between gap-1 rounded-form whitespace-nowrap transition-all duration-200 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 [&[data-state=open]>svg]:rotate-180", "border-b border-neutral-darkest-15 bg-transparent text-scheme-text placeholder:text-neutral-darkest-60 alternate:border-white-10 alternate:bg-transparent alternate:text-white alternate:placeholder:text-white", "min-h-8 py-2", className)} {...props}>
      {children}
